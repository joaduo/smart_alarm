...
class DeferParser:
    ...
    def __call__(self, v):
        return DeferValue(self.parser, v)
...
class DeferValue:
    ...
    def parse(self):
        logger.debug(f'Evaluating {self.value}')
        return self.parser(self.value)
...
class ConfigVar(object):
    ...
    @property
    def env_var(self):
        return self._env_var or 'SMART_ALARM_' + self.name.upper()
...
def solve_settings(module_name=None, file_path=None):
    ...
    if not global_settings:
        file_path = file_path or os.environ.get('SMART_ALARM_SETTINGS_FILE')
        if file_path:
            ...
            module_name = module_name or os.environ.get('SMART_ALARM_SETTINGS_MODULE',
                                                        'smart_alarm.default_settings')
